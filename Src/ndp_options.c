...
ndp_optPayload *ndp_createOptionAdvertisementIntervalOption(uint32_t interval)
{
    ndp_optPayload *option = malloc(NDP_OPT_ADVERTISEMENT_INTERVAL_PROP_LEN * 8);
    option->AdvertisementInterval = htobe32(interval);
    return option;
}

ndp_optPayload *ndp_createOptionRouteInformation(uint8_t prefixLength, uint8_t routePreference, uint32_t routeLifetime, struct in6_addr perfix)
{
    ...
    if (perfixLength != 0)
    {
        propLen += perfixLength / 64; // length in 8 bytes
    }
    ...
    if (perfixLength >= 64)
    {
        ((uint64_t *)(&perfix))[1] = (((uint64_t *)(&perfix))[1] << (128 - perfixLength)) >> (128 - perfixLength);
    }
    if (perfixLength < 64)
    {
        ((uint64_t *)(&perfix))[0] = (((uint64_t *)(&perfix))[0] << (64 - perfixLength)) >> (64 - perfixLength);
    }

    memcpy(subOption->Perfix, &perfix, (propLen - 1) * 8);
